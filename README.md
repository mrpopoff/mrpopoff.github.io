...
__АО «Вяртсиля Цифровые Технологии»__

- технический лидер продукта Unidata Manager
- инженер-программист (Python)

##### Технический лидер Unidata Manager (набор баз данных и сервисов для хранения и обработки больших массивов пространственных данных
- технический аудит существующего решения (фаза MVP)
    - формирование требований по наблюдаемости и управляемости сервисов
    - рекомендации по повышению стабильности длительных операций (pipeline обработки данных)
    - рекомендации по кросс-платформенности разрабатываемых сервисов (API)
    - рекомендации по контейнеризации API
- формирование требований к железу
- создание окружений dev/stage/prod
- вывод сервисов в промышленную эксплуатацию 
- выгрузка логов и метрик в корпоративное облако Azure (Application Insights)
- локальный мониторинг ресурсов stage/prod (Prometheus/Grafana)
- интеграция с облачными сервисами Wartsila
    - интеграция с облачным сервисами  ИБ (Azure AD)
    - синхронизация данных портов и объектов навигации
- проектирование процессов непрерывной интеграции между Unidata и потребителями данных внутри компании
    - формирование понятия артефакта Unidata с результатами обработки пространственных данных (формат дескриптора, формат пакета)
    - оформление результатов всех pipeline обработки данных в виде артефактов Unidata
    - публикация артефактов Unidata в корпоративных облачных сервисах (Artifactory, Azure Blob Storage)
- проектирование автоматического обновления коллекции морских карт (S-57) Unidata по данным, публикуемым UK Hydrographic Office (UKHO)
- проектирование SQL-based хранилища пространственных данных морской картографии с покрытием на весь мир (PostgreSQL + PostGIS)  

##### Инженер-программист Python
- реализация API сервисов Unidata (Django, FastAPI, Sqlalchemy)
- реализация выгрузки метрик и логов в системы мониторинга (переключение между Azure или ELK + Jaeger)
- реализация выгрузки изменений базы данных портов Unidata в Wartsila Ports API
- реализация сервиса загрузки и инкрементального обновления базы данных объектов из коллекции морских карт S-57
- реализация механизма уведомлений пользователей Unidata о существенно изменившихся данных (границы карт, плотность точек и т.д.)
- реализация прототипа сервера векторных тайлов (mbtiles) на основе файловой БД SQLite и СУБД PostgreSQL
